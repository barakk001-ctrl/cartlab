...
- **Price tracker (no AI, no external services)** — each item can carry a shelf price and a product barcode (scanned with the camera via zxing, or typed). Every price change is recorded per product — keyed by barcode when present, so history survives renames and re-adds. After shopping, the "Receipt" button takes the receipt's text (Israeli receipts print `barcode | name | price` per line; copy it off the photo with iOS Live Text and paste) and a regex parser turns the product lines into price history, skipping discount/total lines and refreshing the price on list items whose barcode matches. The "Prices" dashboard shows the latest price per product with the change vs. its previous different price (red ▲ increase, green ▼ decrease), biggest movers first.
- **"Just bought" holding area** — items checked in the last 10 minutes sit in a temporary section right below "To buy", above the older cart items, so what was just grabbed stays in view (and a mis-tap is easy to undo). After 10 minutes they settle into "In cart". The check-off time syncs, so both shoppers see the same grouping.
- **Item notes** — tap an item's name to add a short note ("the brand in the blue package"); it shows under the name and syncs.
- **Urgent items** — the item sheet has a "Mark as urgent" button: everyone sharing the list — including the person who marked it, as confirmation — gets an immediate push notification with the product name (in their own language), and the item jumps to a red 🚨 Urgent section at the top of the list. Devices register for these alerts automatically once notification permission is granted (marking something urgent, or setting a reminder, is the moment the app asks). As long as urgent items remain unbought, every subscribed device also gets a morning digest at 7:00 (in its own timezone) listing them. And since lock screens sort newest-first (nothing can truly be pinned), an unbought urgent item re-sends its notification every 30 minutes for 3 hours — each re-send replaces the previous one (same tag) and jumps back to the top; buying or un-marking the item stops it.
