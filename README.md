...
- **Urgent items** — the item sheet has a "Mark as urgent" button: everyone else sharing the list gets an immediate push notification with the product name (in their own language), and the item jumps to a red 🚨 Urgent section at the top of the list. Devices register for these alerts automatically once notification permission is granted (marking something urgent, or setting a reminder, is the moment the app asks).
