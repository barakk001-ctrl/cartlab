...
- **Receipt scanning & price tracker** — after shopping, "Scan receipt" uploads a photo of the till receipt; the server has Claude (vision) read it — Hebrew or English, thermal print and all — extract each product's final paid unit price, and match lines to the list's item names. The "Prices" dashboard then shows the latest price per product with the change vs. its previous price (red ▲ increase, green ▼ decrease), biggest movers first. Requires `ANTHROPIC_API_KEY` in the server environment (Railway → service → Variables); without it the button reports that scanning isn't configured.
